...
flowchart LR
  ...
  C["Modal CPU API<br/>JWT + specialist validators"]
  G["Modal T4 GPU<br/>RF-DETR-L TensorRT"]
  M["Gemini Flash<br/>borderline object only"]

  B -->|"typed commands; no score"| E
  E --> A
  E -->|"transactional RPC"| D
  B -->|"5 transient JPEG frames"| C
  C -->|"JWKS / owner / active turn"| A
  C -->|"read config; resolve RPC"| D
  C -->|"object batch"| G
  C -.->|"0.45–0.65 only"| M
```
...
sequenceDiagram
  ...
  loop once per second; one request in flight
    B->>V: sequenceNo + token + 5 ROI frames
    V->>D: verify active turn, owner, validator config
    V->>G: object quest only
    alt continue
...
camera → raw MediaStream + 62% square ROI + JPEG batches
...
- `src/features/vision`: multipart transport, DTO validation, overlays, one-in-flight loop.

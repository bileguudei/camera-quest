-- Correction to 202608190003. Object recognition was fine as it was: the frame
-- coverage gate it introduced is removed here, and the validator that read it
-- has been reverted, so objects behave exactly as before.
--
-- The real complaint was colour: a wall or whatever sat behind the player
-- passed a colour turn too easily, so the thresholds go up rather than down.

update public.quests
set validator_config = validator_config - 'minArea' - 'minAreaWhenSeen'
where kind = 'object';

-- Walls, wood and skin are mostly desaturated; a genuinely coloured object is
-- not. Raising the saturation floor is what separates them, and the larger new
-- area plus four-frame consensus stops a slow pan across the room from passing.
update public.quests
...
-- Restore the original draw: a background target is only nudged down the order,
-- not held back to last resort. The portable weighting stays — that was asked
-- for separately and does not change how anything is recognised.
